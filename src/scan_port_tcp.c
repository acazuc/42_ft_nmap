...
void scan_port_tcp(t_thread_arg *thread_arg, struct iphdr *ip_header, void (*forge_tcphdr)(t_env *env, t_tcp_packet *packet, uint16_t port, int pton_addr), uint16_t port, char *type)
{
	...
	while (retry < 3 && !received)
	{
		...
		{
			fprintf(stderr, RED "ft_nmap: failed to send packet\n" DEFAULT);
			exit(EXIT_FAILURE);
		}
		int looper = 0;
		while (looper < 10)
		{
			...
			if (poll(&fds, 0, 100) == -1)
			{
				fprintf(stderr, RED "ft_nmap: poll failed\n" DEFAULT);
				exit(EXIT_FAILURE);
			}
			looper++;
		}
